...
<span style="background-color: #FFE44E; text-align: center; padding: 0 4px; border-radius: 8px;">1번</span>의 경우에는 도메인이 다르면
당연하게 리소스 요청이 되지않습니다. <span style="background-color: #FFE44E; text-align: center; padding: 0 4px; border-radius: 8px;">2번</span>의 경우에는
APIServer를 WebServer가 받게된다면 문제 될것 없습니다. &nbsp; APIServer의 리소스를 단순히 WebServer가 전달하는 역할이라면
&nbsp;<span style="background-color: #FFE44E; text-align: center; padding: 0 4px; border-radius: 8px;">1번</span>의 형태로 요청을 하는게 일반적인 사고일 것입니다.
<br/><br/>

<h3>JSONP(JSON with Padding) SOP를 우회하자)</h3>

JSONP는 JSON데이터를 SOP에 상관없이 불러올 수 있다는 것을 이용하는 방식이다.
JSON데이터라고 해도 직접적으로 HTTPRequest를 할 경우에는 에러가 발생하지만 script태그를 이용한다면 받아올 수 있다.
(허나 JSON데이터 그대로 script태그에 넣으면 에러가 발생함으로 유효한 js문법으로 감싸야하는 번거로움이 생김)
<br/>

<h3> CORS(Cross-origin resource sharing) 공식 허용 방식 </h3>

CORS는 공식적으로 출처가 다른 리소스를 접근할 수 있는 권한을 부여하도록 하는 체제입니다.
따라서 공식적으로 HttpRequest 통하여 진행되며 허용된 요청인지 확인하는 절차가 진행됩니다.
